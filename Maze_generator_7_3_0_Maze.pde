  ...
  for (int xPos = 0; xPos<SizeX+1; xPos++) {
    for (int yPos = 0; yPos<2*SizeY+1; yPos++) {
      if (connections [xPos] [yPos] != 0) {
        ...
        if (explore [xPos] [yPos]!=0 && !((xPos==0 && yPos==1) || (xPos==SizeX-1 && yPos==2*SizeY))) { 
...
void Maze() {
  ...
  popMatrix();
  fill(200, 0, 0);
  for (int t =1; pathKey [t-1] [0] != keyX || pathKey [t-1] [1] != keyY; t++) {      
    pushMatrix();
    translate(-400*pathKey [t] [1], 0, -400*pathKey [t] [0]);
    rotateX(-PI/2);  
    rect(-200, 200, 400, 400);
    popMatrix();
  }
  fill(0, 200, 0);
  for (int t =1; pathEnd [t-1] [0] != SizeX-1 || pathEnd [t-1] [1] != SizeY-1; t++) {      
    pushMatrix();
    translate(-400*pathEnd [t] [1], 0, -400*pathEnd [t] [0]);
    rotateX(-PI/2);  
    rect(-200, 200, 400, 400);
    popMatrix();
  }
